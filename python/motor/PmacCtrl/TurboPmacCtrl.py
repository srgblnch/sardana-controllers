...
__author__ = "Zbigniew Reszela"
__email__ = "zreszela@cells.es"
__copyright__ = "Copyright, CELLS / ALBA Synchrotron"
__license__ = "GPLv3+"
...
class TurboPmacController(MotorController):
    ...
    cs_extra_attributes = \
        {"MotionProgramRunning": {'Type': 'PyTango.DevBoolean',
                                  'R/W Type': 'PyTango.READ'}}
    ...
    def PreStateAll(self):
        ...
        try:
            ...
            self._log.error("PreStateAll(): PmacEth DeviceProxy state "
                            "command failed.")
    ...
    def StateAll(self):
        ...
        for axis in self.axesList:
            # here we will work on reference to dictionary to save time
            # in accessing it.
            ...
            attributes["ExtendedServoAlgorithmEnabled"] = \
                bool(motBinState[0] & 0x1)
            ...
            attributes["AlternateSource_Destination"] = \
                bool(motBinState[4] & 0x8)
            ...
            attributes["AlternateCommand-OutputMode"] = \
                bool(motBinState[5] & 0x1)
            # Second Word
            # We add one because these bits together hold a value equal
            # to the Coordinate System nr minus one.
            attributes["CoordinateSystem"] = motBinState[6] + 1

            attributes["CoordinateDefinition"] = \
                self.translateCoordinateDefinition(motBinState[7])

            attributes["AssignedToCoordinateSystem"] = \
                bool(motBinState[8] & 0x8)
            # Reserved for future use
            attributes["ForegroundInPosition"] = bool(motBinState[8] & 0x2)
            attributes["StoppedOnDesiredPositionLimit"] = \
                bool(motBinState[8] & 0x1)
            ...
            attributes["IntegratedFatalFollowingError"] = \
                bool(motBinState[10] & 0x4)
    ...
    def GetPar(self, axis, name):
        ...
        if name.lower() == "velocity":
            ...
            try:
                ...
                # self._log.error("GetPar(%d,%s,%s): GetIVariable(%d) command "
            ...
            sardanaVelocity /= self.attributes[axis]["step_per_unit"]
    ...
    def AbortOne(self, axis):
        if self.attributes[axis]["MotionProgramRunning"]:
            ...
            try:
                ...
                # self._log.error("AbortOne(%d): OnlineCmd(%s) command "
                #                 "called on PmacEth DeviceProxy failed. \n"
            ...
            try:
                ...
                # self._log.error("AbortOne(%d): JogStop(%d) command called "
                #                 "on PmacEth DeviceProxy failed. \n"
                #                 "Exception: %s" % (axis, axis, e))
                self._log.error("AbortOne(%d): JogStop(%d) command called on "
                                "PmacEth DeviceProxy failed." % (axis, axis))
        ...
        if len_cmd == 1:
            ...
            if len_cmd == 2:
                ...
                if cmd_splitted[0].lower() == "getmvariable":
                    return \
                        str(self.pmacEth.command_inout("getmvariable",
                                                       int(cmd_splitted[1])))
            elif len_cmd > 2:
